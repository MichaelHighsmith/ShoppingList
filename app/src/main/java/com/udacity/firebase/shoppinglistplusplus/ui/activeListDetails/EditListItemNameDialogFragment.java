...
public class EditListItemNameDialogFragment extends EditListDialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        //use EditListDialogFragment #helpSetDefaultValueEditText(String) to set the default text of the TextView
    ...
    protected void doListEdit() {
        ...
        //Set input text to be the current list item name
        if(!nameInput.equals("") && !nameInput.equals(mItemName)){
            Firebase firebaseRef = new Firebase(Constants.FIREBASE_URL);

            //Make a map for the item you are editing
            HashMap<String, Object> updatedItemToAddMap = new HashMap<String, Object>();

            //Add the new name to the update map
            updatedItemToAddMap.put("/" + Constants.FIREBASE_LOCATION_SHOPPING_LIST_ITEMS + "/" + mListId + "/" + mItemId + "/" + Constants.FIREBASE_PROPERTY_ITEM_NAME, nameInput);
